...
function Get-AppCreatorDistributionPointGroups {
    try {
        if (-not $script:SiteCode) { throw "Connect to SCCM first." }
        Set-Location "$($script:SiteCode):\\" -ErrorAction Stop
        return @(Get-CMDistributionPointGroup -ErrorAction Stop | Sort-Object Name | ForEach-Object { [string]$_.Name })
    }
    catch {
        throw "Unable to load Distribution Point Groups: $($_.Exception.Message)"
    }
}

function Start-AppCreatorContentDistribution {
    param(
    ...
    try {
        if (-not $script:SiteCode) { throw "Connect to SCCM first." }
        if ([string]::IsNullOrWhiteSpace($ApplicationName)) { throw "No application is available for content distribution." }
        if ([string]::IsNullOrWhiteSpace($DistributionPointGroupName)) { throw "Select a Distribution Point Group." }

        Set-Location "$($script:SiteCode):\\" -ErrorAction Stop
        $app = Get-CMApplication -Name $ApplicationName -Fast -ErrorAction Stop | Select-Object -First 1
        if (-not $app) { throw "Application '$ApplicationName' was not found in Configuration Manager." }

        Write-Log "[DISTRIBUTE] Starting content distribution for '$ApplicationName' to DP Group '$DistributionPointGroupName'..."
        Start-CMContentDistribution `
            -ApplicationName $ApplicationName `
            -DistributionPointGroupName $DistributionPointGroupName `
            -ErrorAction Stop | Out-Null

        Write-Log "[DISTRIBUTE] Distribution request submitted successfully."
        return [pscustomobject]@{
            Success = $true
            Message = "Content distribution for '$ApplicationName' was submitted to Distribution Point Group '$DistributionPointGroupName'."
        }
    }
    catch {
...
function New-SCCMScriptApplication {
    ...
    try {
        ...
        # Configuration Manager requires a unique Application name.
        # Keep the clean base name for the first version. If that name already
        # exists with another version, create the new release as "Name - Version".
        # This lets multiple releases coexist without touching the older app.
        $targetAppName = $AppName.Trim()
        $baseExisting = Get-CMApplication -Name $targetAppName -Fast -ErrorAction SilentlyContinue | Select-Object -First 1

        if ($baseExisting) {
            $baseVersion = [string]$baseExisting.SoftwareVersion

            if ($baseVersion -eq [string]$Version) {
                throw "Application '$targetAppName' version '$Version' already exists in Configuration Manager. No duplicate was created."
            }

            $targetAppName = "$($AppName.Trim()) - $Version"
            $versionExisting = Get-CMApplication -Name $targetAppName -Fast -ErrorAction SilentlyContinue | Select-Object -First 1
            if ($versionExisting) {
                throw "Application '$targetAppName' already exists in Configuration Manager. No duplicate was created."
            }

            Write-Log "[CREATE] Base application '$AppName' already exists (version '$baseVersion')."
            Write-Log "[CREATE] New version will be created as '$targetAppName'."
        }

        $deploymentTypeName = "$targetAppName - Script Installer"

        New-CMApplication -Name $targetAppName -Publisher $Publisher -SoftwareVersion $Version -ErrorAction Stop | Out-Null

        Write-Log "[CREATE] Application '$targetAppName' created."
        ...
        return [PSCustomObject]@{
            Success = $true
            ApplicationName = $targetAppName
            Mensagem = "Application '$targetAppName' created with Deployment Type and PowerShell Detection Method configured in Configuration Manager."
        }
    }
    catch {
        return [PSCustomObject]@{ Success = $false; Mensagem = $_.Exception.Message }
...
$script:LastCreatedApplicationName = $null
$script:BuildId = '2026.08.27-SCCM-CREATOR-FINAL11-DISTRIBUTION'
...
$form.Size             = New-Object System.Drawing.Size(720, 1005)
...
$grpDistribution = New-Object System.Windows.Forms.GroupBox
$grpDistribution.Text = "6. Content Distribution"
$grpDistribution.Location = New-Object System.Drawing.Point(10, 705)
$grpDistribution.Size = New-Object System.Drawing.Size(690, 70)
$form.Controls.Add($grpDistribution)

$lblDPGroup = New-Object System.Windows.Forms.Label
$lblDPGroup.Text = "Distribution Point Group:"
$lblDPGroup.Location = New-Object System.Drawing.Point(10, 29)
$lblDPGroup.Size = New-Object System.Drawing.Size(145, 20)
$grpDistribution.Controls.Add($lblDPGroup)

$cmbDPGroup = New-Object System.Windows.Forms.ComboBox
$cmbDPGroup.Location = New-Object System.Drawing.Point(160, 25)
$cmbDPGroup.Size = New-Object System.Drawing.Size(300, 24)
$cmbDPGroup.DropDownStyle = 'DropDownList'
$grpDistribution.Controls.Add($cmbDPGroup)

$btnRefreshDPGroups = New-Object System.Windows.Forms.Button
$btnRefreshDPGroups.Text = "Refresh"
$btnRefreshDPGroups.Location = New-Object System.Drawing.Point(470, 24)
$btnRefreshDPGroups.Size = New-Object System.Drawing.Size(75, 27)
$grpDistribution.Controls.Add($btnRefreshDPGroups)

$btnDistributeContent = New-Object System.Windows.Forms.Button
$btnDistributeContent.Text = "Distribute Content"
$btnDistributeContent.Location = New-Object System.Drawing.Point(550, 22)
$btnDistributeContent.Size = New-Object System.Drawing.Size(130, 30)
$btnDistributeContent.BackColor = [System.Drawing.Color]::LightSkyBlue
$btnDistributeContent.Enabled = $false
$grpDistribution.Controls.Add($btnDistributeContent)

# --- Log ---
$txtLog = New-Object System.Windows.Forms.TextBox
$txtLog.Location = New-Object System.Drawing.Point(10, 785)
$txtLog.Size = New-Object System.Drawing.Size(690, 170)
...
$btnConnect.Add_Click({
    ...
    if ($result.Success) {
        ...
        try {
            $cmbDPGroup.Items.Clear()
            foreach ($groupName in @(Get-AppCreatorDistributionPointGroups)) { [void]$cmbDPGroup.Items.Add($groupName) }
            if ($cmbDPGroup.Items.Count -gt 0) {
                $cmbDPGroup.SelectedIndex = 0
                Write-Log "Loaded $($cmbDPGroup.Items.Count) Distribution Point Group(s)."
            } else {
                Write-Log "No Distribution Point Groups were found in this site."
            }
        }
        catch { Write-Log "Distribution Point Group load failed: $($_.Exception.Message)" }
    } else {
...
$btnRefreshDPGroups.Add_Click({
    if (-not $script:SiteCode) {
        [System.Windows.Forms.MessageBox]::Show("Connect to SCCM first.", "Warning", 'OK', 'Warning') | Out-Null
        return
    }
    try {
        $selected = if ($cmbDPGroup.SelectedItem) { [string]$cmbDPGroup.SelectedItem } else { $null }
        $cmbDPGroup.Items.Clear()
        foreach ($groupName in @(Get-AppCreatorDistributionPointGroups)) { [void]$cmbDPGroup.Items.Add($groupName) }
        if ($cmbDPGroup.Items.Count -gt 0) {
            if ($selected -and $cmbDPGroup.Items.Contains($selected)) { $cmbDPGroup.SelectedItem = $selected }
            else { $cmbDPGroup.SelectedIndex = 0 }
        }
        Write-Log "Refreshed Distribution Point Groups: $($cmbDPGroup.Items.Count) found."
    }
    catch {
        Write-Log "ERROR loading Distribution Point Groups: $($_.Exception.Message)"
        [System.Windows.Forms.MessageBox]::Show($_.Exception.Message, "Distribution Point Groups", 'OK', 'Error') | Out-Null
    }
})

$btnDistributeContent.Add_Click({
    if ([string]::IsNullOrWhiteSpace($script:LastCreatedApplicationName)) {
        [System.Windows.Forms.MessageBox]::Show("Create an application first in this session. The tool will distribute the exact application name that was created, including the version suffix when applicable.", "No Application Available", 'OK', 'Warning') | Out-Null
        return
    }
    if (-not $cmbDPGroup.SelectedItem) {
        [System.Windows.Forms.MessageBox]::Show("Select a Distribution Point Group.", "Warning", 'OK', 'Warning') | Out-Null
        return
    }

    $groupName = [string]$cmbDPGroup.SelectedItem
    $confirm = [System.Windows.Forms.MessageBox]::Show(
        "Distribute content for:`n$($script:LastCreatedApplicationName)`n`nTo Distribution Point Group:`n$groupName?",
    ...
    $btnDistributeContent.Enabled = $false
    [System.Windows.Forms.Application]::DoEvents()
    $distResult = Start-AppCreatorContentDistribution -ApplicationName $script:LastCreatedApplicationName -DistributionPointGroupName $groupName
    $btnDistributeContent.Enabled = $true

    if ($distResult.Success) {
        [System.Windows.Forms.MessageBox]::Show(
            $distResult.Message + "`n`nConfiguration Manager will process the distribution in the background. Use Monitoring > Distribution Status > Content Status to track completion.",
            "Distribution Submitted", 'OK', 'Information'
        ) | Out-Null
    } else {
        Write-Log "[DISTRIBUTE] ERROR: $($distResult.Message)"
        [System.Windows.Forms.MessageBox]::Show($distResult.Message, "Distribution Error", 'OK', 'Error') | Out-Null
    }
})
...
$btnCreate.Add_Click({
    ...
    if ($result.Success) {
        $script:LastCreatedApplicationName = [string]$result.ApplicationName
        $btnDistributeContent.Enabled = $true
        Write-Log $result.Mensagem
        Write-Log "[DISTRIBUTE] Ready to distribute '$($script:LastCreatedApplicationName)'."
