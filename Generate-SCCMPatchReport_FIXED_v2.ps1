...
"@
    ...
    foreach ($c in $complianceRows.Rows) {
    ...
    $hosts = foreach ($sys in $systems.Rows) {
...
function New-PatchReport {
    ...
    $hostsJson     = ConvertTo-Json -InputObject @($allHosts) -Depth 5 -Compress
    $bySiteJson    = ConvertTo-Json -InputObject @($bySite) -Depth 5 -Compress
    $byOSJson      = ConvertTo-Json -InputObject @($byOS) -Depth 5 -Compress
    $topErrorsJson = ConvertTo-Json -InputObject @($topErrors) -Depth 5 -Compress
