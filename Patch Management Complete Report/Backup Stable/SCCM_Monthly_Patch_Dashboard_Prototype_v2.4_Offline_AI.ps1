...
function Get-ErrorRecommendations {
    ...
    if (-not $entry -or -not $entry.recommendations) { return '' }
...
function Get-ErrorDetail {
    ...
    foreach ($candidate in @($LastEnforcementMessage, $StatusDescription)) {
        ...
        if ($trimmed -notmatch '^(Failed to install update\(s\)|Enforcement failed|Error|Unknown)$') {
            return $trimmed
        }
    }
...
<th>Operating System</th><th>OS Version</th><th>OS Build Number</th><th>Pending Restart</th><th>Deployment Status</th>
...
<footer>Generated by SCCM Monthly Patch Dashboard Prototype v2.4</footer>
<script>
const data=[
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        ...
        Title="SCCM Monthly Patch Dashboard - Prototype v2.4 Offline AI"
    ...
    <Grid Margin="20">
        ...
        <Grid Grid.Row="3" Margin="0,14,0,0">
            ...
            <StackPanel Grid.Column="1" Orientation="Horizontal" Margin="16,0,0,0" VerticalAlignment="Bottom">
                <Button x:Name="btnKnowledgeBase" Content="Import KB Update..." Width="155" Height="40" Margin="0,0,10,0" ToolTip="Import an offline JSON knowledge-base update. No internet connection is used."/>
...
foreach ($name in @(
    ...
    'progress','txtStatus','btnKnowledgeBase','btnOutput','btnGenerate','btnClose'
)) {
...
$script:ErrorKnowledgeBase = $null
$loadedKnowledgeBase = Read-ErrorKnowledgeBase
if ($loadedKnowledgeBase) {
    $script:LoadedKnowledgeBaseVersion = [string]$loadedKnowledgeBase.knowledgeBaseVersion
}
else {
    $script:LoadedKnowledgeBaseVersion = 'Not loaded'
}
...
$controls.btnKnowledgeBase.Add_Click({
    $dialog = New-Object Microsoft.Win32.OpenFileDialog
    $dialog.Title = 'Select an offline SCCM error knowledge-base update'
    $dialog.Filter = 'Knowledge Base JSON (*.json)|*.json|All files (*.*)|*.*'
    $dialog.CheckFileExists = $true

    if ($dialog.ShowDialog() -eq $true) {
        try {
            $result = Import-ErrorKnowledgeBaseFile -SourcePath $dialog.FileName
            $controls.txtStatus.Text = "Knowledge base $($result.Version) imported successfully with $($result.ErrorCount) error entries."
            [System.Windows.MessageBox]::Show(
                "Offline knowledge base imported successfully.`r`n`r`nVersion: $($result.Version)`r`nErrors: $($result.ErrorCount)`r`n`r`nA backup of the previous base was created automatically.",
