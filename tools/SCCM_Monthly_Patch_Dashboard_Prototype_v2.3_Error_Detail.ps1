...
function Get-DemoData {
    ...
    $rows = for ($i = 0; $i -lt $statuses.Count; $i++) {
        ...
        [pscustomobject]@{
            ...
            ErrorDetail             = if ($status -eq 'Error') { 'Delivery Optimization: Download of a file saw no progress within the defined period.' } else { '' }
...
table{border-collapse:separate;border-spacing:0;width:100%;min-width:2200px;font-size:13px}th,td{padding:10px 12px;border-bottom:1px solid var(--border);text-align:left;white-space:nowrap}th{position:sticky;top:0;background:#edf2f8;z-index:2;cursor:pointer}tr:hover td{background:#f8fbff}.error-detail{white-space:normal;min-width:420px;max-width:680px;line-height:1.35}
...
<th>Error Code</th><th>Error Detail</th><th>Last Status Time</th><th>Resource ID</th>
</tr></thead>
<tbody>
$($tableRows -join "`n")
</tbody>
