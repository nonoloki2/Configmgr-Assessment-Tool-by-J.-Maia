...
<#
.SYNOPSIS
    SCCM Monthly Patch Dashboard - Prototype v1
...
.NOTES
    ...
    Prototype v1 includes Demo Mode so the interface and web report can be tested
...
function applyFilters(){
 ...
 rows.forEach(row=>{
   ...
   const rebootValue=cells[14]?.innerText.trim()||'';
   const activityValue=cells[7]?.innerText.trim()||'';
...
<footer>Generated by SCCM Monthly Patch Dashboard Prototype v1</footer>
<script>
const data=[
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        ...
        Title="SCCM Monthly Patch Dashboard - Prototype v1"
        Height="650" Width="930" MinHeight="620" MinWidth="850"
    ...
    <Grid Margin="22">
        ...
        <StackPanel Grid.Row="0" Margin="0,0,0,16">
            <TextBlock Text="SCCM Monthly Patch Dashboard" FontSize="27" FontWeight="SemiBold" Foreground="#10233F"/>
            <TextBlock Text="Generate a modern clickable HTML dashboard for monthly patch deployments."
                       Margin="0,5,0,0" Foreground="#5B6778" FontSize="14"/>
        </StackPanel>

        <Border Grid.Row="1" Background="White" CornerRadius="14" BorderBrush="#D9E2EC" BorderThickness="1" Padding="22">
            <Grid>
                ...
                <TextBlock Grid.Row="0" Grid.Column="0" Text="SMS Provider server" VerticalAlignment="Center" Margin="0,0,12,12"/>
                <TextBox x:Name="txtProvider" Grid.Row="0" Grid.Column="1" Margin="0,0,12,12" Height="32" Padding="8,5"/>
                <Button x:Name="btnTest" Grid.Row="0" Grid.Column="2" Content="Test connection" Height="32" Margin="0,0,0,12"/>

                <TextBlock Grid.Row="1" Grid.Column="0" Text="Site code" VerticalAlignment="Center" Margin="0,0,12,12"/>
                <TextBox x:Name="txtSiteCode" Grid.Row="1" Grid.Column="1" Margin="0,0,12,12" Height="32" Padding="8,5" Text="PR1"/>
                <TextBlock Grid.Row="1" Grid.Column="2" Text="Example: PR1" Foreground="#718096" VerticalAlignment="Center" Margin="0,0,0,12"/>

                <TextBlock Grid.Row="2" Grid.Column="0" Text="Assignment ID" VerticalAlignment="Center" Margin="0,0,12,12"/>
                <TextBox x:Name="txtAssignmentID" Grid.Row="2" Grid.Column="1" Margin="0,0,12,12" Height="32" Padding="8,5"/>
                <TextBlock Grid.Row="2" Grid.Column="2" Text="Numeric ID" Foreground="#718096" VerticalAlignment="Center" Margin="0,0,0,12"/>

                <TextBlock Grid.Row="3" Grid.Column="0" Text="Output folder" VerticalAlignment="Center" Margin="0,0,12,12"/>
                <TextBox x:Name="txtOutput" Grid.Row="3" Grid.Column="1" Margin="0,0,12,12" Height="32" Padding="8,5"/>
                <Button x:Name="btnBrowse" Grid.Row="3" Grid.Column="2" Content="Browse..." Height="32" Margin="0,0,0,12"/>

                <TextBlock Grid.Row="4" Grid.Column="0" Text="Options" VerticalAlignment="Top" Margin="0,3,12,12"/>
                <StackPanel Grid.Row="4" Grid.Column="1" Grid.ColumnSpan="2" Margin="0,0,0,12">
                    <CheckBox x:Name="chkDemo" Content="Demo mode — generate sample data without SCCM" IsChecked="True" Margin="0,0,0,8"/>
                    <CheckBox x:Name="chkUpn" Content="Resolve UPN through Active Directory when necessary" IsChecked="True" Margin="0,0,0,8"/>
                    <CheckBox x:Name="chkReboot" Content="Query pending reboot live on every device (slower)" IsChecked="False"/>
                </StackPanel>

                <TextBlock Grid.Row="5" Grid.Column="0" Text="Report behavior" VerticalAlignment="Top" Margin="0,3,12,12"/>
                <TextBlock Grid.Row="5" Grid.Column="1" Grid.ColumnSpan="2" TextWrapping="Wrap" Foreground="#526172" Margin="0,0,0,14"
                           Text="Each donut segment opens a separate browser tab with a searchable, sortable device list. The same action is available from status cards and legend items."/>

                <Separator Grid.Row="6" Grid.ColumnSpan="3" Margin="0,4,0,14"/>

                <TextBlock Grid.Row="7" Grid.Column="0" Text="Progress" VerticalAlignment="Top" Margin="0,3,12,8"/>
                <ProgressBar x:Name="progress" Grid.Row="7" Grid.Column="1" Grid.ColumnSpan="2" Height="18" Minimum="0" Maximum="100" Margin="0,0,0,8"/>

                <TextBox x:Name="txtStatus" Grid.Row="8" Grid.ColumnSpan="3" IsReadOnly="True"
                         TextWrapping="Wrap" VerticalScrollBarVisibility="Auto" AcceptsReturn="True"
                         Background="#F8FAFC" BorderBrush="#D9E2EC" Padding="10"
                         Text="Ready. Demo mode is enabled so you can test the complete web export immediately."/>
            </Grid>
        </Border>

        <StackPanel Grid.Row="2" Orientation="Horizontal" HorizontalAlignment="Right" Margin="0,16,0,0">
            <Button x:Name="btnGenerate" Content="Generate dashboard" Width="180" Height="42"
                    Background="#2563EB" Foreground="White" FontWeight="SemiBold" Margin="0,0,10,0"/>
            <Button x:Name="btnClose" Content="Close" Width="100" Height="42"/>
        </StackPanel>
    </Grid>
</Window>
'@
...
$controls = @{}
foreach ($name in @('txtProvider','txtSiteCode','txtAssignmentID','txtOutput','btnTest','btnBrowse',
                     'chkDemo','chkUpn','chkReboot','progress','txtStatus','btnGenerate','btnClose')) {
    $controls[$name] = $window.FindName($name)
}

$defaultOutput = Join-Path ([Environment]::GetFolderPath('Desktop')) 'SCCM_Patch_Reports'
$controls.txtOutput.Text = $defaultOutput

$controls.chkDemo.Add_Checked({
    $controls.txtProvider.IsEnabled = $false
    $controls.txtSiteCode.IsEnabled = $false
    $controls.txtAssignmentID.IsEnabled = $false
    $controls.btnTest.IsEnabled = $false
    $controls.txtStatus.Text = 'Demo mode enabled. The report will use sample devices and statuses.'
})
$controls.chkDemo.Add_Unchecked({
    $controls.txtProvider.IsEnabled = $true
    $controls.txtSiteCode.IsEnabled = $true
    $controls.txtAssignmentID.IsEnabled = $true
    $controls.btnTest.IsEnabled = $true
    $controls.txtStatus.Text = 'Real SCCM mode enabled. Enter the SMS Provider, site code and Assignment ID.'
})
...
$controls.btnBrowse.Add_Click({
    ...
    $dialog.SelectedPath = $controls.txtOutput.Text
    if ($dialog.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) {
        $controls.txtOutput.Text = $dialog.SelectedPath
    }
})

$controls.btnTest.Add_Click({
    try {
        $provider = $controls.txtProvider.Text.Trim()
        $siteCode = $controls.txtSiteCode.Text.Trim()
        if (-not $provider -or -not $siteCode) { throw 'Enter the SMS Provider server and site code.' }
        $controls.txtStatus.Text = 'Testing SMS Provider connection...'
        $window.Dispatcher.Invoke([action]{}, 'Background')
        $ns = "root\SMS\site_$siteCode"
        $null = Get-CimInstance -ComputerName $provider -Namespace $ns -ClassName SMS_ProviderLocation `
            -OperationTimeoutSec 30 -ErrorAction Stop | Select-Object -First 1
        $controls.txtStatus.Text = "Connection successful: $provider / $ns"
        [System.Windows.MessageBox]::Show('SMS Provider connection was successful.','Connection test','OK','Information') | Out-Null
    }
    catch {
        $controls.txtStatus.Text = "Connection failed: $($_.Exception.Message)"
...
$controls.btnGenerate.Add_Click({
    ...
    try {
        $output = $controls.txtOutput.Text.Trim()
        if ([string]::IsNullOrWhiteSpace($output)) { throw 'Select an output folder.' }
        New-Item -ItemType Directory -Path $output -Force | Out-Null

        $tempLog = Join-Path $env:TEMP ("SCCM_Patch_Dashboard_{0}.log" -f ([guid]::NewGuid().ToString('N')))
        Set-Content -LiteralPath $tempLog -Value '' -Encoding UTF8

        if ([bool]$controls.chkDemo.IsChecked) {
            $controls.txtStatus.Text = 'Creating demo dataset...'
            $controls.progress.Value = 25
            $window.Dispatcher.Invoke([action]{}, 'Background')

            $demo = Get-DemoData
            $rows = $demo.Rows
            $deploymentName = $demo.DeploymentName
            $collectionName = $demo.CollectionName
            $collectionID = $demo.CollectionID
            $assignmentID = $demo.AssignmentID
            $deploymentID = $demo.DeploymentID
            Write-Log $tempLog 'Demo mode report generated.'
        }
        else {
            $provider = $controls.txtProvider.Text.Trim()
            $siteCode = $controls.txtSiteCode.Text.Trim().ToUpperInvariant()
            $assignmentText = $controls.txtAssignmentID.Text.Trim()

            if (-not $provider) { throw 'Enter the SMS Provider server.' }
            if (-not $siteCode) { throw 'Enter the site code.' }
            $assignmentID = 0
            if (-not [int]::TryParse($assignmentText, [ref]$assignmentID)) {
                throw 'Assignment ID must be numeric.'
            }

            $controls.txtStatus.Text = 'Reading deployment and device status from the SMS Provider...'
            $controls.progress.Value = 15
            $window.Dispatcher.Invoke([action]{}, 'Background')

            $providerData = Get-SmsProviderData -ProviderServer $provider -SiteCode $siteCode `
                -AssignmentID $assignmentID -LogPath $tempLog

            $controls.txtStatus.Text = "Enriching $($providerData.Assets.Count) device records..."
            $controls.progress.Value = 40
            $window.Dispatcher.Invoke([action]{}, 'Background')

            $rows = Convert-AssetsToRows -Assets $providerData.Assets -ResourceMap $providerData.ResourceMap `
                -SiteCode $siteCode -ResolveUpnEnabled ([bool]$controls.chkUpn.IsChecked) `
                -PendingRebootEnabled ([bool]$controls.chkReboot.IsChecked) -LogPath $tempLog

            $firstAsset = $providerData.Assets | Select-Object -First 1
            $summary = $providerData.Summary
            $deploymentName = if ($firstAsset.AssignmentName) { [string]$firstAsset.AssignmentName } else { "Assignment $assignmentID" }
            $collectionName = if ($firstAsset.CollectionName) { [string]$firstAsset.CollectionName } elseif ($summary) { [string]$summary.CollectionName } else { '' }
            $collectionID = if ($firstAsset.CollectionID) { [string]$firstAsset.CollectionID } elseif ($summary) { [string]$summary.CollectionID } else { '' }
            $deploymentID = if ($summary) { [string]$summary.DeploymentID } else { [string]$firstAsset.AssignmentUniqueID }
        }

        $controls.txtStatus.Text = 'Generating HTML pages and CSV files...'
        $controls.progress.Value = 75
        $window.Dispatcher.Invoke([action]{}, 'Background')

        $result = Export-ReportPackage -Rows $rows -DeploymentName $deploymentName `
            -CollectionName $collectionName -CollectionID $collectionID `
            -AssignmentID ([string]$assignmentID) -DeploymentID $deploymentID `
            -BaseOutputFolder $output -LogPath $tempLog

        $controls.progress.Value = 100
        $controls.txtStatus.Text = "Completed.`r`nDashboard: $($result.DashboardPath)`r`nDevices: $($rows.Count)"
        Start-Process $result.DashboardPath

        [System.Windows.MessageBox]::Show(
            "Dashboard generated successfully.`r`n`r`n$($result.ReportFolder)",
            'Report completed','OK','Information'
        ) | Out-Null
    }
    catch {
        $controls.progress.Value = 0
        $controls.txtStatus.Text = "Error: $($_.Exception.Message)"
        [System.Windows.MessageBox]::Show($_.Exception.Message,'Dashboard generation error','OK','Error') | Out-Null
    }
    finally {
        $controls.btnGenerate.IsEnabled = $true
    }
})
